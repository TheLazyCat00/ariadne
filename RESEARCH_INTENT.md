...
4. **Explicit authorization for write-capable lab workflows**
   - Any future IL patch writer should be developed and tested against owned fixtures or explicitly authorized challenge binaries.
   - A fixture manifest should identify the sample, its hash, its authorization status, and whether write-capable testing is allowed.

5. **No stealth or persistence features**
...
6. **Clear output labeling**
   - Symbolic candidates, dry-run patch plans, and unverified hypotheses must be labeled as such.

## Proposed fixture-manifest model

Write-capable experimental backends should require an explicit fixture manifest for lab use. Example:

```json
{
  "name": "owned-dotnet-il-gate-demo",
  "description": "Locally generated .NET IL challenge fixture for solver and patcher regression tests.",
  "sha256": "<expected sample hash>",
  "authorization": "Owned/generated by the researcher for this project.",
  "allow_solve": true,
  "allow_patch_write": true,
  "notes": "No third-party commercial code. No license circumvention."
}
```

This kind of manifest is not a substitute for real authorization, but it makes lab intent explicit and helps prevent accidental use of write-capable tooling on inappropriate targets.
